...
SCRIPT="$REPO/cfo-shared/scripts/notify_watch.py"
PY="/usr/bin/python3"
...
mkdir -p "$HOME/Library/LaunchAgents" "$HOME_DIR/logs" "$HOME_DIR/inbox"
cat > "$PLIST" <<XML
...
cat <<TXT
  ...
  1. Full Disk Access for the python that reads Notification Center:
       ...
       $PY
     (press ⌘⇧G in the file dialog and paste that path)
...
TXT
if "$PY" "$SCRIPT" --check --home "$HOME_DIR" 2>/dev/null; then
    ok "Notification Center is readable now -- forwarding starts with the next bank alert"
else
    warn "not readable yet -- expected until toggle 1 is done; check with:"
